...
Ask: "Which message should we handle first?"
...
3. Present to user with recommendation:
   - **Accept** — deviation is minor or an improvement
   - **Reject** — deviation breaks downstream
...
2. Ask the user what this is and how they want it routed. Typical outcomes:
   - Identify it as one of the named types → handle via that branch
   - Treat as a complex item → track via `node tools/dev-tools.cjs inbox add <number> "<title>"` for later planning work
