...
// "<message>"` and reads the printed report.
//
...
//
// Three subcommands replace the inline `gh issue create / comment / edit /
// close` heredocs (formerly hand-rolled):
//
//   issue-create     --from <handoff.json> --index <i>
//   issue-edit-body  --from <handoff.json> --index <i>
...
// issueEditBody — replace an issue body from a handoff entry { op, issue, body }.
function issueEditBody() {
  const { entry, op } = loadHandoffEntry('issue-edit-body', 'UPDATE_TASK');
  const fail = (msg) => { console.log(`${op}: ✗ ${msg}`); process.exit(1); };
  ...
  const edited = gh(['issue', 'edit', issue, '--body-file', '-'], { input: body });
  if (!edited.ok) fail(`gh issue edit failed: ${ghReason(edited)}`);

  // Verify — the fetched body matches what we sent (CRLF-normalized).
  const norm = (s) => (s || '').replace(/\r\n/g, '\n').trim();
  const verifyOnce = () => {
    const v = gh(['issue', 'view', issue, '--json', 'body', '--jq', '.body']);
    return v.ok && norm(v.out) === norm(body);
  };
  if (!verifyOnce()) { sleepMs(2000); if (!verifyOnce()) fail(`#${issue} edit sent but verification failed (body mismatch or gh view error). Investigate before retrying.`); }

  console.log(`${op} #${issue}: ✓ body updated and verified.`);
  process.exit(0);
}
...
switch (command) {
  ...
  case 'issue-transition':
    ...
    break;
  default:
    console.log('Usage: node "$AGENT_DIR/tools/dev-tools.cjs" <hook|git-check|territory-check|check-files|inbox|workflow-check|next-step|clear-draft|clear-techstack-notes|commit|push|board-drags|issue-create|issue-edit-body|issue-transition>');
}
