...
input=$(cat)
[ -z "$input" ] && { printf "Spec Agent"; exit 0; }
command -v jq >/dev/null 2>&1 || { printf "Spec Agent"; exit 0; }
...
# proj_hash keys the per-project inbox cache further down.
proj_hash=""
...
if command -v gh >/dev/null 2>&1 && [ -n "$GH_REPO" ]; then
    ...
    if [ -n "$inbox_cache" ] && [ -f "$inbox_cache" ]; then
        ...
        [ $(( $(date +%s) - imt )) -lt 60 ] && { inbox_fresh=true; c_pm=$(cat "$inbox_cache"); }
    fi

    if ! $inbox_fresh; then
        c_pm=$(gh issue list --label "for-pm" --state open --json number 2>/dev/null | jq 'length' 2>/dev/null || echo 0)
        [ -n "$inbox_cache" ] && echo "$c_pm" > "$inbox_cache" 2>/dev/null
    fi
fi

[ "${c_pm:-0}" -gt 0 ] 2>/dev/null && { [ -n "$line2" ] && line2+="${sep}"; line2+="${orange}${c_pm} for-pm${reset}"; }
...
if [ -n "$PROJECT_DIR" ]; then
    git_status=$(git -C "$PROJECT_DIR" status --porcelain -- cowmoo/specs/ cowmoo/agent-files/pm/ 2>/dev/null)
