---
name: design-draft
description: Compose task bodies for the batch agreed in /design-start, validate via @design-task-checker, write to design-draft.md. Rerunnable.
user-invocable: true
...
Compose the design task bodies for the batch agreed in `/design-start`. Each task body has two sections — short Instructions for the human + dense self-contained Claude Design Prompt. After composition, validate the draft mechanically and refine until clean. Output: `cowmoo/agent-files/uxui/design-draft.md`.
...
## Step 4: Write preliminary draft.md

Once all task bodies are composed (in conversation), write the draft to `$PROJECT_DIR/cowmoo/agent-files/uxui/design-draft.md` so the checker can read it:

```markdown
# Design Draft

_Generated by `/design-draft` on <date>. Run `/design-publish` to ship._

## Batch context
- Why these together: <coherence reason from /design-start>
- Inherits: <visual direction inherited>
- Sets up: <what this batch establishes for downstream>
...
[the composed task body — Instructions + Claude Design Prompt]
...
Re-read the draft you just wrote. Verify:
- [ ] Batch context section present
- [ ] All N task bodies present in agreed order
- [ ] Each task body has both Instructions and Claude Design Prompt sections
- [ ] No content silently dropped or truncated
...
Spawn `@design-task-checker` (no args needed — the sub-agent reads `cowmoo/agent-files/uxui/design-draft.md` directly).
...
- Edit the relevant task body inline (Edit the draft.md directly, scoped to the affected section)
...
- If the second check still reports **REFINE** → surface the remaining findings to the user and **stop**. Do NOT attempt another automatic fix pass. Message: "After one fix pass, the checker still reports: [findings]. Edit `design-draft.md` directly or re-run `/design-draft` to recompose from scratch."
...
Saved to cowmoo/agent-files/uxui/design-draft.md.
