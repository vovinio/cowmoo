...
- **UI-level divergences are marked and logged** — when a screen definition makes a deliberate UI-level call that runs ahead of the spec (a state, layout, or interaction the spec doesn't cover but that is the right product call), add a `**Spec divergence:**` line to that screen and log a `For: PM` entry to `PENDING-CORRECTIONS.md`, per `.claude/rules/corrections.md`. A genuine business-logic gap still goes to `/ask pm`, not the queue. Conversely, if a screen carries a `**Spec divergence:**` marker the spec has since caught up to, clear the stale marker.
