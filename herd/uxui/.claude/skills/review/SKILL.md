...
- **Non-blocking copy deltas are deferred, not escalated** — a copy-grade mismatch surfaced during coverage review (microcopy, a label, wording drifted from a spec update but still mapping to the same element) is logged to `PENDING-CORRECTIONS.md` per `.claude/rules/corrections.md`, not routed to the blocking `/ask pm` lane. The Step 3b "Spec gap" classification is for *missing or contradictory spec content* — a genuine blocker — not for deferrable wording.
- **Design ahead of spec is a divergence, not an orphan** — when coverage review finds the design embodies a deliberate UI-level decision the spec doesn't cover (an "orphaned" screen or element that is a sound extension, not cruft), treat it as a spec divergence per `.claude/rules/corrections.md`: ensure the screen carries a `**Spec divergence:**` marker and log a `For: PM` entry. Distinct from a genuine coverage gap (spec content with no UI) and from a stale orphan to prune. Conversely, if a screen still carries a `**Spec divergence:**` marker but the spec has since caught up to it, clear the stale marker — design and spec now agree.
