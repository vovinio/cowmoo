...
| **PM** | The comments contest the task's *spec premise* in a way that **breaks the product** — a question only PM can answer, design cannot proceed without it. (A *non-breaking* designer decision that runs ahead of the spec is a spec divergence — logged, not escalated; see below.) | closed, **no** `uxui:done` | `removeLabel: "uxui:review"`, `close: true`, `comment` (cross-links the escalation) | escalate via `/ask pm` — the loop closes through the `for-pm` issue |
...
- *Comment contests the spec* — a designer says the screen shouldn't exist *and the spec breaks without it* → **PM acts next**. Close (no `uxui:done`) and `/ask pm`. But a designer comment that merely records a deliberate decision running ahead of the spec is a **spec divergence** — logged, not escalated (see below).

**A deferrable copy delta — and a designer-led spec divergence — is not a reject.** A copy-grade finding (microcopy, a label, wording drift) and a *deliberate, sound* designer divergence from the brief (a layout, state, role, or interaction the designer chose differently) are both logged to `PENDING-CORRECTIONS.md` (see `.claude/rules/corrections.md`) and the bundle still takes the **design approved** row — neither sends the card back. Only a designer **omission or error** (a missing state, a broken interaction) or a divergence that **breaks the product** triggers the designer-acts-next row. `/dispatch-corrections` later ships the logged items — copy deltas as a batched `uxui:todo`, spec divergences to PM for alignment.
