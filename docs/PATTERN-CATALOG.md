...
- Sender skill — a skill that composes a cross-agent message and invokes `issue-create`: `/notify` (announcements), `/ask` (blocked requests), `/dispatch-corrections` (batched deferred non-blocking corrections). Takes target as argument.
